...
FCC_COMMANDS="fcc-desktop fcc-server fcc-claude fcc-codex fcc-pi fcc-qwen fcc-init free-claude-code"
...
configure_and_verify_free_claude_code() {
    ...
    if [ "$dry_run" -eq 1 ]; then
        ...
        printf '+ verify fcc-desktop, fcc-server, fcc-claude, fcc-codex, fcc-pi, and fcc-qwen in the uv tool bin directory\n'
    ...
    for command_name in fcc-desktop fcc-server fcc-claude fcc-codex fcc-pi fcc-qwen; do
